...
def assess_ai_likelihood(items: list[dict], subject: str) -> dict:
    """Judge whether a student's written answers were likely AI-generated.
    items = [{question, answer}]. Returns {verdict, notes}. A soft signal, not proof."""
    if not items:
        return {"verdict": "uncertain", "notes": "No written answers to assess."}
    joined = "\n\n".join(
        f"Q: {(i.get('question') or '')[:400]}\nA: {(i.get('answer') or '')[:1500]}" for i in items[:10]
    )
    prompt = f"""You are analysing a {subject} student's exam answers to judge whether they were likely written by the student or generated by an AI assistant.

Signals of AI authorship: unnatural fluency/polish, textbook-perfect structure, generic phrasing, no personal voice or working shown, vocabulary far above typical student level, or answers that ignore the specific wording of the question. Be fair — strong students exist — so only flag genuinely suspicious patterns, and prefer "uncertain" when unsure.

ANSWERS:
{joined}

Return ONLY JSON:
{{"verdict": "likely_ai" | "likely_human" | "uncertain", "notes": "1-3 sentences of specific reasoning for the teacher"}}"""
    for attempt in range(2):
        try:
            ...
            if verdict not in ("likely_ai", "likely_human", "uncertain"):
                verdict = "uncertain"
            return {"verdict": verdict, "notes": (data.get("notes") or "").strip()[:1000]}
        except Exception:
            if attempt == 1:
                return {"verdict": "uncertain", "notes": "Could not analyse automatically."}
